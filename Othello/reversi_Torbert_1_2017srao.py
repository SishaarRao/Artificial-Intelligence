...
def genPossibilities(myboard, mypiece):
   ...
   for pos in range (0, len(myboard)):
      if myboard[pos] == mypiece:         
         for j in pattern:
            if surrounded(myboard, mypiece, pos, pattern[j]):
...
def moveAlphaBeta(myboard, depth, alpha, beta, player):
   ...
   if player == theX:
      ...
      for a in poss:
         tempboard = myboard
         tempboard[a] = theX
         moveVal = (moveAlphaBeta(myboard, depth - 1, alpha, beta, theO))[0]
         if temp > moveVal:
            temp = moveVal
            move = a
         alpha = max(alpha, temp)
      ...
      for a in poss:
         tempboard = myboard
         tempboard[a] = theO
         moveVal = (moveAlphaBeta(myboard, depth - 1, alpha, beta, theX))[0]
         if temp < moveVal:
            temp = moveVal
            move = a
         beta = min(beta, temp)
...
#
   ...
   myboard[28] = theO
   myboard[35] = theO
   print(moveAlphaBeta(myboard, 0, -1000, 1000, mypiece))
   #myboard[val] = mypiece
   display(myboard)
#
##################################################
